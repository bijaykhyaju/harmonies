                      ...
                      <div class="button-section">
                        ...
                        <div class="left-element">
                          ...
                          <select type="select" id="select-instrument" name="instrument" class="custom-select" style="width:40%">
                              ...
                              @foreach(App\MusicInstrument::getAllInstruments() as $instruments)

                                <option value="{{$instruments->id}}" @if($instrumentResult->id == $instruments->id) {{"selected='selected'"}} @endif >{{$instruments->name}}</option>
                      ...
                      <div class="table-container" id="form-field-music">
                        ...
                        <form id="beats_form">
                              ...
                              <?php
                                  ...
                                  //
                                  // if($instrumentResult->name == 'Piano'){
                                  //   $url_base = 'tonejs/audio/piano/';
                                  //   $note_extension = ".mp3";
                                  // }
                                  // if($instrumentResult->name == 'Flute'){
    ...
    <!-- @include('includes/footer') -->
          ...
          <script>
            ...
            $(document).ready(function(){
                  //$(".beats-table").scrollLeft('500px');


                  //load the instrument
                  $('#select-instrument').change(function(){
                    $('.gooey').fadeIn(200);
                    $('#beats_form').addClass('loading-div');
                    var inst_id = $(this).val();
                    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
                    $.ajax({
                      url: '{{ url("/music/select-instrument")}}',
                      type: 'post',
                      data: { _token : CSRF_TOKEN, instrtrument_id: inst_id},
                      error: function(data) {
